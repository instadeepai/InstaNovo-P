...
from instanovo.utils.minLoRA import add_lora
from instanovo.utils.minLoRA import LoRAParametrization
...
class InstaNovo(nn.Module):
    ...
    def __init__(
        ...
        if apply_lora:
            add_lora(self.encoder, self.lora_config)
            add_lora(self.decoder, self.lora_config)
    ...
    def forward(
        ...
        return self._decoder(x, p, y, x_mask, y_mask, add_bos), y
    ...
    def init(
        ...
        # logits = self._decoder(x, p, None, x_mask, None, add_bos=False)
    ...
    def get_eos_index(self) -> int:
        ...
        return self.residue_set.EOS_INDEX  # type: ignore

    def get_empty_index(self) -> int:
        """Get the PAD token ID."""
        return self.residue_set.PAD_INDEX  # type: ignore
...
if __name__ == "__main__":
    ...
    # import os
    # import numpy as np
    # from instanovo.utils.minLoRA import get_lora_params
    # from instanovo.utils.minLoRA import get_lora_state_dict

    # config_path = os.path.join(
    #     os.path.dirname(os.path.realpath(__file__)),
    #     "../../configs/instanovo/instanovo_finetune_phosphor.yaml",
    # )

    # with open(config_path) as f_in:
    #     config = yaml.safe_load(f_in)
    # config["residues"] = {str(aa): float(mass) for aa, mass in config["residues"].items()}
    # residue_set = residue_set = ResidueSet(
    ...
    #     },
    # )
    # model = InstaNovo(residue_set=residue_set, apply_lora=True)
    # # print(model)

    # model_state_dict = model.state_dict()
    # print(model_state_dict.keys())

    # # Use regex to get the keys of the model_state_dict ending in "*.lora_A"
    # state_dict_lora_a_keys = [k for k in model_state_dict.keys() if k.endswith(".lora_A")]
    # # print(state_dict_lora_a_keys)
    ...
    # print(f"model: {np.sum([p.numel() for p in model.parameters()]):,d} parameters")
    # print(f"model.head: {np.sum([p.numel() for p in model.head.parameters()]):,d} parameters")
    # print(
    #     f"model.aa_embed.weight: {np.sum([p.numel() for p in model.aa_embed.weight]):,d} parameters"
    # )
    # print(f"lora: {np.sum([p.numel() for p in parameters[0]['params']]):,d} parameters")
    pass
